...
CREATE TABLE `donhang` (
  ...
  `SDT` varchar(500) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL,
  `TongTien` float DEFAULT NULL,
  `NgayMua` date DEFAULT NULL,
  `TrangThai` bit(1) DEFAULT NULL,
  `MaKH` int DEFAULT NULL,
  `MaTT` int DEFAULT NULL,
  `MaVC` int DEFAULT NULL,
  PRIMARY KEY (`MaDH`),
  KEY `MaKH` (`MaKH`),
  KEY `MaTT` (`MaTT`),
  KEY `MaVC` (`MaVC`),
  CONSTRAINT `donhang_ibfk_1` FOREIGN KEY (`MaKH`) REFERENCES `khachhang` (`MaKH`),
  CONSTRAINT `donhang_ibfk_2` FOREIGN KEY (`MaTT`) REFERENCES `thanhtoan` (`MaTT`),
  CONSTRAINT `donhang_ibfk_3` FOREIGN KEY (`MaVC`) REFERENCES `vanchuyen` (`MaVC`)
